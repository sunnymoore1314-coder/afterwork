...
export default function ResultPage(){
 ...
 return <div className="site-shell"><Brand/><main className="result-main"><Link href="/" className="back-link"><ArrowLeft size={16}/>Back to check-in</Link><div className="result-intro"><p className="eyebrow">TONIGHT BELONGS TO YOU</p><h1>Your evening,<br/><em>a little softer.</em></h1></div><article className="result-card"><div className="result-card-top"><span><Sparkles size={16}/>{source==="example"?"YOUR EXAMPLE PLAN":"YOUR RECOVERY PLAN"}</span><span className="result-end">{context.start_time} — {clockAt(context.start_time,context.minutes)}{tomorrow?" · ends tomorrow":""}</span></div><h2 lang={chinese?"zh-CN":"en"}>{plan.title}</h2><div className="result-meta"><span><Clock3 size={15}/>{context.minutes} min</span><span><Wallet size={15}/>{context.budget===0?"Free":"Under ¥"+context.budget}</span><span><MapPin size={15}/>{context.preference==="indoors"?"Indoors preferred":context.preference==="outside"?"Outside preferred":"Flexible"}</span>{context.city&&<span>{context.city}</span>}{context.weather&&<span><Cloud size={15}/>{context.weather}</span>}</div><div className="result-goal"><span>{chinese?"今晚的小目标":"A LITTLE INTENTION"}</span><h3>{plan.goal}</h3><p>{plan.summary}</p></div><RecoveryTimeline steps={plan.steps}/><div className="closing-note"><Moon size={20}/><blockquote>{plan.final_message}</blockquote><span>— afterwork</span></div>{source==="manual"&&<p className="example-disclosure">Imported from the AI chat you chose. Afterwork validated the structure and calculated the timeline.</p>}{source==="example"&&<p className="example-disclosure">An example from our activity library, adapted to your choices.</p>}</article><div className="result-after"><span>You can skip a step. You can stop early. This time is yours.</span><Link href="/">Adjust my evening <ArrowLeft size={15}/></Link></div></main><Footer/></div>;
}