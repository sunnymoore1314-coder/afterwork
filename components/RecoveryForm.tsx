...
import { LoaderCircle, ChevronDown, Plus, AlertCircle, LocateFixed, CloudSun } from "lucide-react";
...
function locate(){setDetailsOpen(true);setError("");if(!navigator.geolocation){setError(t("locationUnsupported"));return}setLocationLoading(true);navigator.geolocation.getCurrentPosition(async position=>{const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),15000);try{setCity(t("currentLocation"));setWeather(await weatherAtPosition(position.coords.latitude,position.coords.longitude,language,controller.signal))}catch{setError(t("weatherFailed"))}finally{clearTimeout(timer);setLocationLoading(false)}},()=>{setError(t("locationFailed"));setLocationLoading(false)},{enableHighAccuracy:false,timeout:12000,maximumAge:600000})}
async function fillWeather(){if(!city.trim()||city===t("currentLocation"))return;const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),12000);setLocationLoading(true);try{const result=await weatherForCity(city.trim(),language,controller.signal);setCity(result.city);setWeather(result.weather)}catch{setError(t("cityWeatherFailed"))}finally{clearTimeout(timer);setLocationLoading(false)}}
